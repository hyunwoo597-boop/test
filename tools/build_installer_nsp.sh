...
nacptool --create \
  "RE5 Korean + Character Mod Installer" \
  "RE5 Mod Installer Project" \
  "17.2.0" \
  "${TARGET}.nacp" \
  --titleid="$TITLEID"
...
test -s "$ICON" || { echo "custom installer icon missing: $ICON" >&2; exit 4; }
...
cp "$INSTALLER_NSP" RE5_Korean_Mod_Installer.nsp
test -s RE5_Korean_Mod_Installer.nsp

echo "Installer NSP built: $(stat -c %s RE5_Korean_Mod_Installer.nsp) bytes"
